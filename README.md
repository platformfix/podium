...
An SPDX software bill of materials is generated and attested the same way, pinned to the exact image digest rather than a floating tag. Download it for offline inspection:

```bash
gh attestation download oci://ghcr.io/platformfix/podium:latest --owner platformfix
```

The Helm chart releases get the same treatment: every chart version is attested too, not just the image.
